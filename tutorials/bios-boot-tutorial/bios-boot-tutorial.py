...
def startWallet():
    ...
    background(args.kpaybchaind + ' --unlock-timeout %d --http-server-address 127.0.0.1:6666 --wallet-dir %s' % (unlockTimeout, os.path.abspath(args.wallet_dir)))
    sleep(.4)
    run(args.clpaybchain + 'wallet create --to-console')

def importKeys():
    run(args.clpaybchain + 'wallet import --private-key ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clpaybchain + 'wallet import --private-key ' + key)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clpaybchain + 'create account eosio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clpaybchain + 'system newaccount --transfer eosio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clpaybchain + 'transfer eosio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clpaybchain + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clpaybchain + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.clpaybchain + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.clpaybchain + 'get table eosio eosio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clpaybchain + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.clpaybchain + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clpaybchain + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.clpaybchain + 'push action eosio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clpaybchain + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clpaybchain + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    run(args.clpaybchain + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'eosio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clpaybchain + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clpaybchain + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clpaybchain + 'push action eosio buyrambytes' + jsonArg(['eosio', accounts[0]['name'], 200000]) + '-p eosio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clpaybchain + 'create key --to-console')
...
def stepKillAll():
    run('killall kpaybchaind nodpaybchain || true')
    sleep(1.5)
def stepStartWallet():
    startWallet()
    importKeys()
...
def stepInstallSystemContracts():
    run(args.clpaybchain + 'set contract eosio.token ' + args.contracts_dir + '/eosio.token/')
    run(args.clpaybchain + 'set contract eosio.msig ' + args.contracts_dir + '/eosio.msig/')
def stepCreateTokens():
    run(args.clpaybchain + 'push action eosio.token create \'["eosio", "10000000000.0000 %s"]\' -p eosio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.clpaybchain + 'push action eosio.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.clpaybchain + 'set contract eosio ' + args.contracts_dir + '/eosio.system/')
    sleep(1)
    run(args.clpaybchain + 'push action eosio setpriv' + jsonArg(['eosio.msig', 1]) + '-p eosio@active')
def stepInitSystemContract():
    run(args.clpaybchain + 'push action eosio init' + jsonArg(['0', '4,SYS']) + '-p eosio@active')
    sleep(1)
...
commands = [
    ('k', 'kill',               stepKillAll,                True,    "Kill all nodpaybchain and kpaybchaind processes"),
    ('w', 'wallet',             stepStartWallet,            True,    "Start kpaybchaind, create wallet, fill with keys"),
...
parser.add_argument('--clpaybchain', metavar='', help="Cleos command", default='../../build/programs/clpaybchain/clpaybchain --wallet-url http://127.0.0.1:6666 ')
parser.add_argument('--nodpaybchain', metavar='', help="Path to nodpaybchain binary", default='../../build/programs/nodpaybchain/nodpaybchain')
parser.add_argument('--kpaybchaind', metavar='', help="Path to kpaybchaind binary", default='../../build/programs/kpaybchaind/kpaybchaind')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clpaybchain')
...
args.clpaybchain += '--url http://127.0.0.1:%d ' % args.http_port

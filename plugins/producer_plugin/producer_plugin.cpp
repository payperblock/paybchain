...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      fc::microseconds                                          _kpaybchaind_provider_timeout_us;
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "   KEOSD:<data>    \tis the URL where kpaybchaind is available and the approptiate wallet(s) are unlocked")
         ("kpaybchaind-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowed for sending blocks to a kpaybchaind provider for signing")
...
static producer_plugin_impl::signature_provider_type
make_kpaybchaind_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kpaybchaind_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kpaybchaind_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), fc::optional<uint16_t>());
   else
      kpaybchaind_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kpaybchaind_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kpaybchaind_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kpaybchaind_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kpaybchaind_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            if (spec_type_str == "KEY") {
               ...
               my->_signature_providers[pubkey] = make_kpaybchaind_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kpaybchaind_provider_timeout_us = fc::milliseconds(options.at("kpaybchaind-provider-timeout").as<int32_t>());

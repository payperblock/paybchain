...
{
   try {
      app().set_version(eosio::nodpaybchain::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "eosio/nodpaybchain/data" );
      app().set_default_config_dir(root / "eosio/nodpaybchain/config" );
      ...
      initialize_logging();
      ilog("nodpaybchain version ${ver}", ("ver", app().version_string()));
      ilog("eosio root is ${root}", ("root", root.string()));
      ilog("nodpaybchain using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodpaybchain data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
   ...
   ilog("nodpaybchain successfully exiting");
   return SUCCESS;
}

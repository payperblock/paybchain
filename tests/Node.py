...
class Node(object):
    ...
    # Get actions mapped to an account (clpaybchain get actions)
    ...
    def getAccountEosBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clpaybchain get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
    ...
    def getAccountEosBalance(self, scope):
        """Returns SYS currency0000 account balance from clpaybchain get table command. Returned balance is an integer e.g. 980311. """
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clpaybchain get info."""
        if not self.enableMongo:

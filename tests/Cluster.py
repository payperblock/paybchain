...
class Cluster(object):
    ...
    # walletd [True|False] Is kpaybchaind running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        """Cluster container.
        walletd [True|False] Is wallet kpaybchaind running. If not load the wallet plugin
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodeosArgs: string of arguments to pass through to each nodoes instance (via --nodpaybchain flag on launcher)
        ...
        specificExtraNodeosArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodpaybchain flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodpaybchain default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodpaybchainArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodpaybchainArgs += " --plugin eosio::wallet_api_plugin"
        if self.enableMongo:
            nodpaybchainArgs += " --plugin eosio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodeosArgs is not None:
            assert(isinstance(extraNodeosArgs, str))
            nodpaybchainArgs += extraNodeosArgs
        if Utils.Debug:
            nodpaybchainArgs += " --contracts-console"

        if nodpaybchainArgs:
            cmdArr.append("--nodpaybchain")
            cmdArr.append(nodpaybchainArgs)
        ...
        if specificExtraNodeosArgs is not None:
            ...
            for nodeNum,arg in specificExtraNodeosArgs.items():
                ...
                cmdArr.append("--specific-nodpaybchain")
                cmdArr.append(arg)
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodpaybchain instances. allInstances will kill all nodpaybchain instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodpaybchain instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodpaybchain instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)

...
getHeadBlockNum()
{
  INFO="$(programs/clpaybchain/clpaybchain get info)"
  verifyErrorCode "clpaybchain get info"
...
# stand up nodpaybchain cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodpaybchain \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/eosio-launcher/eosio-launcher --nodpaybchain "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clpaybchain/clpaybchain --port $port2 get block 1)"
    verifyErrorCode "clpaybchain get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clpaybchain/clpaybchain create key)"
verifyErrorCode "clpaybchain create key"
...
PASSWORD_INITA="$(programs/clpaybchain/clpaybchain wallet create --name inita)"
verifyErrorCode "clpaybchain wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clpaybchain/clpaybchain wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clpaybchain wallet import"
programs/clpaybchain/clpaybchain wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clpaybchain wallet import"
programs/clpaybchain/clpaybchain wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clpaybchain wallet import"
programs/clpaybchain/clpaybchain wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clpaybchain wallet import"
...
ACCOUNT_INFO="$(programs/clpaybchain/clpaybchain create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clpaybchain create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clpaybchain/clpaybchain get account testera)"
verifyErrorCode "clpaybchain get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clpaybchain/clpaybchain transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clpaybchain transfer"
   ...
   TRANS_INFO="$(programs/clpaybchain/clpaybchain --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clpaybchain get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done

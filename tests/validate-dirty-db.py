...
###############################################################
# Test for validating the dirty db flag sticks repeated nodpaybchain restart attempts
###############################################################
...
def runNodeosAndGetOutput(myTimeout=3):
    """Startup nodpaybchain, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodpaybchain process.")
    cmd="programs/nodpaybchain/nodpaybchain --config-dir etc/eosio/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    ...
    output={}
    try:
        if debug: Print("Setting nodpaybchain process timeout.")
        ...
        Print("ERROR: Nodeos is running beyond the defined wait time. Hard killing nodpaybchain instance.")
...
try:
    ...
    Print("Restart nodpaybchain repeatedly to ensure dirty database flag sticks.")
    timeout=6
    ...
    for i in range(1,4):
        ...
        if not ret[0]:
            errorExit("Failed to startup nodpaybchain successfully on try number %d" % (i))
        assert(ret[1])
